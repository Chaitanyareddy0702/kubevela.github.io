...
The `register-output-demo` component below is a minimal `ComponentDefinition` (Deployment workload, two parameters) that self-registers via `init()` and exercises all four output methods. The Go on the left was compiled and registered into a real `vela-go-definitions` module, applied to a k3d cluster with `vela def apply-module`, and consumed by an `Application` that reached `phase: running` with the workload `1/1 ready`. The output panes show the actual bytes that `comp.ToCue()`, `comp.ToCueWithImports(...)`, `comp.ToParameterSchema()`, and `comp.ToYAML()` return for this component — captured by running a small `go run` driver and pasted verbatim.
